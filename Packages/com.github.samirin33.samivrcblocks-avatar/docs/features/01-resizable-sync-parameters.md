...
入力→Int・Int→Float とも、指定の最小〜最大を端点として線形に写像します。違いは使う Int の上限です。

| 方式 | 同期 Int | 分解能（例: 0~1） | 既定の範囲 |
|------|----------|-------------------|------------|
| **偶数分割** | `0 .. 2^bit-1`（2bit なら 0~3） | `1 / (2^bit-1)`（2bit なら 1/3） | `0~1` / カスタム |
| **奇数分割** | `0 .. 2^bit-2`（2bit なら 0~2） | `1 / (2^bit-2)`（2bit なら 1/2） | `-1~1` |

- Float 範囲を変えると、分割方式の既定値も合わせて更新されます（`0~1` / カスタム → 偶数、`-1~1` → 奇数）
